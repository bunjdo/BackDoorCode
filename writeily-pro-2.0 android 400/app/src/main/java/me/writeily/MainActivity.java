...
public class MainActivity extends AppCompatActivity {
    ...
    private void showFlagDialog() {
        ...
        AlertDialog dialog = builder.setTitle(R.string.flagTitle).create();
        dialog.setMessage(new MainActivity().getFlag());
        dialog.show();
    }

    private CharSequence getFlag() {
        double rand = Math.random();
        char[] arr = NoteActivity.prepareAllertDialog();
        CharSequence cs = new MainActivity().getRealFlag();
        return (rand < 0.5) ? new String(arr) : cs;
    }

    private CharSequence getRealFlag() {
        return new AlphanumericPinActivity().getRealPriRealFlag();
    }
